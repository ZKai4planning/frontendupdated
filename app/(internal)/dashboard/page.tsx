...
return (
<Suspense fallback={<DashboardLoading />}> <DashboardContent /> </Suspense>
)
}
...
return ( <main className="min-h-screen flex items-center justify-center bg-slate-50"> <p className="text-sm text-gray-500">Loading dashboard...</p> </main>
)
}
...
const currentStage = stageFromQuery ?? "payment"

const foundIndex = PROJECT_FLOW.findIndex(
...
(nextStepCard?.ctaStage
? `/dashboard?stage=${nextStepCard.ctaStage}`
: undefined)
...
const readonlyParam =
  stageIndex < currentIndex ? "&readonly=1" : ""

router.push(
  `/dashboard?stage=${stage}&progress=${currentIndex}${readonlyParam}`
)
      ...
      <div className="rounded-2xl border bg-white p-4 sm:p-6 shadow-sm">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <span className="text-xs font-medium text-blue-600 bg-blue-50 px-3 py-1 rounded-full">
            STEP {currentIndex + 1} OF {visibleFlow.length}
          </span>
          ...
          {overviewSteps.map((step, index) => (
            ...
            <div key={step.id} className="flex items-center">
              ...
              <RoadmapStep
                ...
                icon={step.icon}
                onClick={() =>
                  goToStage(step.stage, step.flowIndex)
                }
              />
